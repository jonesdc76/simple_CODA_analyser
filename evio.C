 ...
 *
 * Revision History:
 *   Revision 1.2.1 2019/12/2 Donald Jones
 *   Fixed compile issues relating to size of pointers on 64 bit architecture
 *
 *   $Log: evio.C,v $
...
#ifndef VXWORKS
int evopen_(char *filename,char *flags,EVFILE *handle,int fnlen,int flen)
{
  char *fn, *fl;
  int status;
...
// }
  ...
  case 'r': case 'R': {
    ...
    handle->rw = EV_READ;
    if (handle->file) {
      fread(header,sizeof(header),1,handle->file); /* update: check nbytes return */
      if (header[EV_HD_MAGIC] != EV_MAGIC) {
	temp = int_swap_byte(header[EV_HD_MAGIC]);
	if(temp == EV_MAGIC) {
	  handle->byte_swapped = 1;
	} else {
	  fclose(handle->file);
	  return(S_EVFILE_BADFILE); 
	}
      } else {
	handle->byte_swapped = 0;
      }

      if (handle->byte_swapped) {
	blk_size = int_swap_byte(header[EV_HD_BLKSIZ]);
	handle->buf = (int *) malloc(blk_size*4);
      } else {
	handle->buf = (int *) malloc(header[EV_HD_BLKSIZ]*4);
      }
	...
	return(S_EVFILE_ALLOCFAIL);
      ...
      if(handle->byte_swapped){
	swapped_intcpy(handle->buf,(char *)header,EV_HDSIZ*4);
	fread(&(handle->buf[EV_HDSIZ]),4,blk_size-EV_HDSIZ,handle->file);
      } else {
	memcpy(handle->buf,header,EV_HDSIZ*4);
	fread(handle->buf+EV_HDSIZ,4,
	      header[EV_HD_BLKSIZ]-EV_HDSIZ,
	      handle->file);		/* read rest of block */
      }

      handle->next = handle->buf + (handle->buf)[EV_HD_START];
      handle->left = (handle->buf)[EV_HD_USED] - (handle->buf)[EV_HD_START];
    }
    break;
  case 'w': case 'W':
    ...
    handle->rw = EV_WRITE;
    if (handle->file) {
      handle->buf = (int *) malloc(EVBLOCKSIZE*4);
      if (!(handle->buf)) {
	return(S_EVFILE_ALLOCFAIL);
      }
      handle->buf[EV_HD_BLKSIZ] = EVBLOCKSIZE;
      handle->buf[EV_HD_BLKNUM] = 0;
      handle->buf[EV_HD_HDSIZ] = EV_HDSIZ;
      handle->buf[EV_HD_START] = 0;
      handle->buf[EV_HD_USED] = EV_HDSIZ;
      handle->buf[EV_HD_VER] = EV_VERSION;
      handle->buf[EV_HD_RESVD] = 0;
      handle->buf[EV_HD_MAGIC] = EV_MAGIC;
      handle->next = handle->buf + EV_HDSIZ;
      handle->left = EVBLOCKSIZE - EV_HDSIZ;
      handle->evnum = 0;
    }
    break;
  default:
  ...
  if (handle->file) {
    handle->magic = EV_MAGIC;
    handle->blksiz = handle->buf[EV_HD_BLKSIZ];
    handle->blknum = handle->buf[EV_HD_BLKNUM];
    return(S_SUCCESS);
  } else {
    handle = 0;
    #ifdef DEBUG
...
#ifndef VXWORKS
int evread_(EVFILE *handle,int *buffer,int *buflen)
{
...
#ifndef VXWORKS
int evwrite_(EVFILE *handle,int *buffer)
{
...
#ifndef VXWORKS
int evioctl_(EVFILE *handle,char *request,void *argp,int reqlen)
{
  char *req;
  int status;
 ...
 *****************************************************************/
int evOpenSearch(EVFILE *handle, int *b_handle)
{
  EVFILE *a;
  EVBSEARCH *b;
